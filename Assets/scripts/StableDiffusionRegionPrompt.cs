...
public class StableDiffusionRegionPrompt : MonoBehaviour
{
    ...
    public Dictionary<string, string[]> ResultLLM = new Dictionary<string, string[]>
    {
        ...
        ["Prompt"] = new string[]
        {
            ...
            "加入提示詞會影響圖片生成的結果，現在我會給你兩張圖片，分別是沒加(正確答案提示詞)和有加(正確答案提示詞)的圖片，請解釋如何判斷出這張圖片並不是加了(選擇詳解提示詞)提示詞，解釋的時候用原圖和加了提示詞的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在50個字以內，解釋用[因此可以判斷這並不是加入了(選擇詳解提示詞)後的結果]來做結尾"
        }
    };
}
