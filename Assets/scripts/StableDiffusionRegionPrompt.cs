...
public class StableDiffusionRegionPrompt : MonoBehaviour
{
    ...
    public Dictionary<string, string[]> ResultLLM = new Dictionary<string, string[]>
    {
        ...
        {
            "ReV Animated這個模型生成的圖片風格會介於動畫和現實之間，現在我會給你一張由ReV Animated生成的圖片，請解釋為什麼這張圖片是由ReV Animated這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖是用ReV Animated生成的]來做結尾",
            "ReV Animated這個模型生成的圖片風格會介於動畫和現實之間，現在我會給你一張由ReV Animated生成的圖片，請解釋為什麼這張圖片不是由ReV Animated這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖不是用ReV Animated生成的]來做結尾"
        },

        ["CuteYukiMix"] = new string[]
        {
            "CuteYukiMix這個模型生成的圖片是動畫風格，且生成的圖片會很可愛，現在我會給你一張由CuteYukiMix生成的圖片，請解釋為什麼這張圖片是由CuteYukiMix這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖是用CuteYukiMix生成的]來做結尾",
            "CuteYukiMix這個模型生成的圖片風格會很可愛，現在我會給你一張由CuteYukiMix生成的圖片，請解釋為什麼這張圖片不是由CuteYukiMix這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖不是用CuteYukiMix生成的]來做結尾"
        },

        ["Counterfeit"] = new string[]
        {
            "Counterfeit這個模型生成的圖片是動畫風格，生成的圖片會線條明顯，色彩鮮豔且對比度高，現在我會給你一張由Counterfeit生成的圖片，請解釋為什麼這張圖片是由Counterfeit這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖是用Counterfeit生成的]來做結尾",
            "Counterfeit這個模型生成的圖片是動畫風格，生成的圖片會線條明顯，色彩鮮豔，現在我會給你一張由Counterfeit生成的圖片，請解釋為什麼這張圖片不是由Counterfeit這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖不是用Counterfeit生成的]來做結尾"
        },

        ["Cetus-Mix"] = new string[]
        {
            "Cetus-Mix這個模型生成的圖片是動畫風格，生成的圖片會較柔和、細緻，現在我會給你一張由Cetus-Mix生成的圖片，請解釋為什麼這張圖片是由Cetus-Mix這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖是用Cetus-Mix生成的]來做結尾",
            "Cetus-Mix這個模型生成的圖片是動畫風格，生成的圖片會較柔和、細緻，現在我會給你一張由Cetus-Mix生成的圖片，請解釋為什麼這張圖片不是由Cetus-Mix這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖不是用Cetus-Mix生成的]來做結尾"
        },

        ["DreamShaper"] = new string[]
        {
            "DreamShaper這個模型生成的圖片是現實風格，現在我會給你一張由DreamShaper生成的圖片，請解釋為什麼這張圖片是由DreamShaper這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖是用DreamShaper生成的]來做結尾",
            "DreamShaper這個模型生成的圖片是現實風格，現在我會給你一張由DreamShaper生成的圖片，請解釋為什麼這張圖片不是由DreamShaper這個模型生成的，解釋的長度限制在75個字以內，解釋用[因此可以判斷這張圖不是用DreamShaper生成的]來做結尾"
        },

        // ---------------------LoRa---------------------
        ["Hanfu"] = new string[]
        {
            "Hanfu LoRa在做的事是讓模型生成的漢服更加精細且更符合歷史中的漢服，現在我會給你兩張圖片，分別是沒用Hanfu LoRa微調和有用Hanfu LoRa微調後的結果，請解釋如何判斷出這張經過Hanfu LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Hanfu LoRa微調後的結果]來做結尾",
            "Hanfu LoRa在做的事是讓模型生成的漢服更加精細且更符合歷史中的漢服，現在我會給你一張沒透過Hanfu LoRa微調的圖片，請解釋如何判斷出這張沒經過Hanfu LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Hanfu LoRa微調後的結果]來做結尾"
        },

        ["Cutecat"] = new string[]
        {
            "Cutecat LoRa在做的事是讓模型生成的貓更正常且更可愛，現在我會給你兩張圖片，分別是沒用Cutecat LoRa微調和有用Cutecat LoRa微調後的結果，請解釋如何判斷出這張經過Cutecat LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Cutecat LoRa微調後的結果]來做結尾",
            "Cutecat LoRa在做的事是讓模型生成的貓更正常且更可愛，現在我會給你一張沒透過Cutecat LoRa微調的圖片，請解釋如何判斷出這張沒經過Cutecat LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Cutecat LoRa微調後的結果]來做結尾"
        },

        ["Blindbox"] = new string[]
        {
            "Blindbox LoRa在做的事是讓模型生成的人物變成Q版盲盒娃娃，現在我會給你兩張圖片，分別是沒用Blindbox LoRa微調和有用Blindbox LoRa微調後的結果，請解釋如何判斷出這張經過Blindbox LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Blindbox LoRa微調後的結果]來做結尾",
            "Blindbox LoRa在做的事是讓模型生成的人物變成Q版盲盒娃娃，現在我會給你一張沒透過Blindbox LoRa微調的圖片，請解釋如何判斷出這張沒經過Blindbox LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Blindbox LoRa微調後的結果]來做結尾"
        },

        ["Eye"] = new string[]
        {
            "Eye LoRa在做的事是讓模型生成的圖片變成重點描繪眼睛，讓整張圖片都是眼睛，現在我會給你兩張圖片，分別是沒用Eye LoRa微調和有用Eye LoRa微調後的結果，請解釋如何判斷出這張經過Eye LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Eye LoRa微調後的結果]來做結尾",
            "Eye LoRa在做的事是讓模型生成的圖片變成重點描繪眼睛，讓整張圖片都是眼睛，現在我會給你一張沒透過Eye LoRa微調的圖片，請解釋如何判斷出這張沒經過Eye LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Eye LoRa微調後的結果]來做結尾"
        },

        ["MoXin"] = new string[]
        {
            "MoXin LoRa在做的事是讓模型生成的圖片變成中國水墨畫風格，現在我會給你兩張圖片，分別是沒用MoXin LoRa微調和有用MoXin LoRa微調後的結果，請解釋如何判斷出這張經過MoXin LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過MoXin LoRa微調後的結果]來做結尾",
            "MoXin LoRa在做的事是讓模型生成的圖片變成中國水墨畫風格，現在我會給你一張沒透過MoXin LoRa微調的圖片，請解釋如何判斷出這張沒經過MoXin LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過MoXin LoRa微調後的結果]來做結尾"
        },

        ["Ghibli"] = new string[]
        {
            "Ghibli LoRa在做的事是讓模型生成的圖片變成吉卜力動畫風格，現在我會給你兩張圖片，分別是沒用Ghibli LoRa微調和有用Ghibli LoRa微調後的結果，請解釋如何判斷出這張經過Ghibli LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Ghibli LoRa微調後的結果]來做結尾",
            "Ghibli LoRa在做的事是讓模型生成的圖片變成吉卜力動畫風格，現在我會給你一張沒透過Ghibli LoRa微調的圖片，請解釋如何判斷出這張沒經過Ghibli LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Ghibli LoRa微調後的結果]來做結尾"
        },

        ["Foodphoto"] = new string[]
        {
            "Foodphoto LoRa在做的事是讓模型生成的圖片的食物變得更精緻、細節更多、看起來更美味，現在我會給你兩張圖片，分別是沒用Foodphoto LoRa微調和有用Foodphoto LoRa微調後的結果，請解釋如何判斷出這張經過Foodphoto LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Foodphoto LoRa微調後的結果]來做結尾",
            "Foodphoto LoRa在做的事是讓模型生成的圖片的食物變得更精緻、細節更多、看起來更美味，現在我會給你一張沒透過Foodphoto LoRa微調的圖片，請解釋如何判斷出這張沒經過Foodphoto LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Foodphoto LoRa微調後的結果]來做結尾"
        },

        ["Lineart"] = new string[]
        {
            "Lineart LoRa在做的事是讓模型生成的圖片變成偏向黑白的漫畫風格且線條感會明顯，現在我會給你兩張圖片，分別是沒用Lineart LoRa微調和有用Lineart LoRa微調後的結果，請解釋如何判斷出這張經過Lineart LoRa的微調，解釋的時候用原圖和經過微調後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Lineart LoRa微調後的結果]來做結尾",
            "Lineart LoRa在做的事是讓模型生成的圖片變成偏向黑白的漫畫風格且線條感會明顯，現在我會給你一張沒透過Lineart LoRa微調的圖片，請解釋如何判斷出這張沒經過Lineart LoRa的微調，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Lineart LoRa微調後的結果]來做結尾"
        },

        // ---------------------ControlNet---------------------
        ["depth"] = new string[]
        {
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行Depth controlnet控制的參考圖和1張經過Depth controlnet控制後的圖片，請解釋如何判斷出這張圖片經過Depth controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Depth controlnet控制後的結果]來做結尾",
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行(填入正確答案) controlnet控制的參考圖和1張經過(填入正確答案) controlnet控制後的圖片，請解釋如何判斷出這張圖片不是經過Depth controlnet的控制而是經過(填入正確答案) controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Depth controlnet控制後的結果]來做結尾"
        },

        ["openpose"] = new string[]
        {
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行Openpose controlnet控制的參考圖和1張經過Openpose controlnet控制後的圖片，請解釋如何判斷出這張圖片經過Openpose controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Openpose controlnet控制後的結果]來做結尾",
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行(填入正確答案) controlnet控制的參考圖和1張經過(填入正確答案) controlnet控制後的圖片，請解釋如何判斷出這張圖片不是經過Openpose controlnet的控制而是經過(填入正確答案) controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Openpose controlnet控制後的結果]來做結尾"
        },

        ["canny"] = new string[]
        {
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行Canny controlnet控制的參考圖和1張經過Canny controlnet控制後的圖片，請解釋如何判斷出這張圖片經過Canny controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Canny controlnet控制後的結果]來做結尾",
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行(填入正確答案) controlnet控制的參考圖和1張經過(填入正確答案) controlnet控制後的圖片，請解釋如何判斷出這張圖片不是經過Canny controlnet的控制而是經過(填入正確答案) controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Canny controlnet控制後的結果]來做結尾"
        },

        ["shuffle"] = new string[]
        {
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行Shuffle controlnet控制的參考圖和1張經過Shuffle controlnet控制後的圖片，請解釋如何判斷出這張圖片經過Shuffle controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是經過Shuffle controlnet控制後的結果]來做結尾",
            "我現在會告訴你4種controlnet控制模式的特色，並給你1張用來進行(填入正確答案) controlnet控制的參考圖和1張經過(填入正確答案) controlnet控制後的圖片，請解釋如何判斷出這張圖片不是經過Shuffle controlnet的控制而是經過(填入正確答案) controlnet的控制，解釋的時候用參考圖和經過控制後的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並不是經過Shuffle controlnet控制後的結果]來做結尾"
        },

        // ---------------------Resolution---------------------
        ["128"] = new string[]
        {
            "128x128解析度生成的圖片會模糊且缺少細節，現在我會給你一張128x128解析度的圖片，請解釋為什麼這張圖片的解析度為128x128，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度為128]來做結尾",
            "128x128解析度生成的圖片會模糊且缺少細節，現在我會給你一張不是128x128解析度的圖片，請解釋為什麼這張圖片的解析度不是128x128，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度不是128]來做結尾"
        },

        ["384"] = new string[]
        {
            "384x384解析度生成的圖片會缺少細節，現在我會給你一張384x384解析度的圖片，請解釋為什麼這張圖片的解析度為384x384，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度為384]來做結尾",
            "384x384解析度生成的圖片會缺少細節，現在我會給你一張不是384x384解析度的圖片，請解釋為什麼這張圖片的解析度不是384x384，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度不是384]來做結尾"
        },

        ["512"] = new string[]
        {
            "512x512解析度生成的圖片通常不會有問題，但有時會缺少圖片細節，現在我會給你一張512x512解析度的圖片，請解釋為什麼這張圖片的解析度為512x512，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度為512]來做結尾",
            "512x512解析度生成的圖片通常不會有問題，但有時會缺少圖片細節，現在我會給你一張不是512x512解析度的圖片，請解釋為什麼這張圖片的解析度不是512x512，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度不是512]來做結尾"
        },

        ["1024"] = new string[]
        {
            "1024x1024解析度生成的圖片有時會多出許多不需要的內容，現在我會給你一張1024x1024解析度的圖片，請解釋為什麼這張圖片的解析度為1024x1024，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度為1024]來做結尾",
            "1024x1024解析度生成的圖片有時會多出許多不需要的內容，現在我會給你一張不是1024x1024解析度的圖片，請解釋為什麼這張圖片的解析度不是1024x1024，解釋的長度限制在75個字以內，解釋用[因此可以判斷圖片的解析度不是1024]來做結尾"
        },

        // ---------------------提示詞---------------------
        ["Prompt"] = new string[]
        {
            "加入提示詞會影響圖片生成的結果，現在我會給你兩張圖片，分別是沒加(正確答案提示詞)和有加(正確答案提示詞)的圖片，請解釋如何判斷出這張圖片多加了(正確答案提示詞)提示詞，解釋的時候用原圖和加了提示詞的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這是加入了(正確答案提示詞)後的結果]來做結尾",
            "加入提示詞會影響圖片生成的結果，現在我會給你兩張圖片，分別是沒加(正確答案提示詞)和有加(正確答案提示詞)的圖片，請解釋如何判斷出這張圖片並不是加了(選擇詳解提示詞)提示詞，解釋的時候用原圖和加了提示詞的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在75個字以內，解釋用[因此可以判斷這並是加入了(選擇詳解提示詞)後的結果]來做結尾"
        }
    };
}
