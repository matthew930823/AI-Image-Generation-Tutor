...
public class StableDiffusionRegionPrompt : MonoBehaviour
{
    ...
    public IEnumerator HandlePromptAndGenerateImageForHardMode(System.Action<string[]> callback)
    {
        ...
        Debug.Log("還敢偷看答案:"+ Resolution+ "(" + Addresult + ":2)," + Prompt+ checkpoint+ ""+ randControlnet);
    ...
    public IEnumerator GenerateImageForMultipleChoice(int Width,int Height,string prompt,string Model_checkpoint,string Lora_Name, string controlNetType,string controlnetModule, string base64Image,int seed, System.Action<Texture2D> callback)
    {
        ...
        var requestData = new Txt2ImgRequest
        {
            ...
            negative_prompt = (Lora_Name == "Snoopy")? "NSFW, (worst quality:2), (low quality:2), (normal quality:2), lowres, normal quality, ((grayscale)), acnes, skin blemishes, age spot, (ugly:1.331), (duplicate:1.331), watermark jpeg artifacts signature watermark username blurry" : ((Lora_Name == "漫畫") ? "easynegative, (badhandv4:1.2), NSFW, watermark jpeg artifacts signature watermark username blurry" : "easynegative, (badhandv4:1.2), NSFW, (worst quality:2), (low quality:2), (normal quality:2), lowres, normal quality, ((monochrome)), ((grayscale)), skin spots, acnes, skin blemishes, age spot, (ugly:1.331), (duplicate:1.331), watermark jpeg artifacts signature watermark username blurry, Stable_Yogis_SD1.5_Negatives-neg"),
            override_settings = new Dictionary<string, object>
            {
                { "sd_model_checkpoint", Model_checkpoint },
                { "CLIP_stop_at_last_layers", 2 }
    ...
    IEnumerator ReadFileAndSendPrompt(string LoRa_name)
    {
        ...
        if(LoRa_name == "HardMode")
        {
            fileContent = fileContent.Replace(
                "提示詞數量為7-12個",
                "提示詞數量為5個，不能包含square image這種沒有意義的提示詞"
            );
        }
    ...
    IEnumerator ReadFileAndSendImformation()
    {
        ...
        //Debug.Log(fileContent+ AddLLM);
        // 呼叫 Gemini API 並傳入檔案內容
        yield return StartCoroutine(geminiAPI.SendRequest(fileContent+"輸入"+Prompt , (result) =>
        {
            Match match = Regex.Match(result, @"\{([^}]*)\}");
            if (match.Success)
            {
    ...
    public Dictionary<string, string[]> ResultLLM = new Dictionary<string, string[]>
    {
        ...
        ["Prompt"] = new string[]
        {
            ...
            "加入提示詞會影響圖片生成的結果，現在我會給你兩張圖片，分別是沒加(正確答案提示詞)和有加(正確答案提示詞)的圖片，請解釋如何判斷出這張圖片並不是加了(選擇詳解提示詞)提示詞，解釋的時候用後面的圖片來進行代稱(不允許用其他代稱，例第一張圖片、第二章圖片這種說法不被允許)，解釋的長度限制在50個字以內，解釋用[因此可以判斷這並不是加入了(選擇詳解提示詞)後的結果]來做結尾"
        }
    };
}
